...
def verify_valid(sigma: List[str], list_sigma: List[str], states: List[str], transitions: List[str]) -> bool:
    ...
    for currentTransition in transitions:
        if ((currentTransition[0] not in strippedStates) or (currentTransition[1] not in sigma) or (currentTransition[2] not in list_sigma and currentTransition[2] != 'epsilon')): # verificare conditii
            return False
        elif (transitions[currentTransition][0] not in strippedStates or (transitions[currentTransition][1] not in list_sigma and transitions[currentTransition][1] != 'epsilon') or (transitions[currentTransition][2] not in list_sigma and transitions[currentTransition][2] != 'epsilon')): #conditii verificare
            return False
    return True  # daca nu se returneaza nimic pana acum => date valide => True
